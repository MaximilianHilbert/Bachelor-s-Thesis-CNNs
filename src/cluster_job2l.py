...
if DEBUG:
    learning_rate, noise_level, num_filter_1, num_filter_2, kernel_size_1, kernel_size_2, batch_size=0.001, 0.2, 27, 51, 9, 8, 256
    N_EPOCH = 2
    DEBUG_PATH="debug/"
    ...
    num_filter_1 = int(float(sys.argv[3]))
    num_filter_2 = int(float(sys.argv[4]))
    kernel_size_1 = int(float(sys.argv[5]))
    kernel_size_2 = int(float(sys.argv[6]))
    batch_size = int(float(sys.argv[7]))
    N_EPOCH = 150
    DEBUG_PATH=""

num_units=(np.floor((np.floor((N_REFL-kernel_size_1+1)/2)-kernel_size_2+1)/2))*num_filter_2
SAVE_STRING = f'{learning_rate}_{noise_level}_{num_filter_1}_{num_filter_2}_{kernel_size_1}_{kernel_size_2}_{batch_size}_{current_time}'
logs = os.path.join("logs_new_metric2l/", DEBUG_PATH, f"{SAVE_STRING}")

print("Hyperparameters used")
print(SAVE_STRING)

# Hyperparameter Training Configuration
HP_LEARNING_RATE = hp.HParam("learning_rate", hp.Discrete([learning_rate]))
...
def testing_loss(model):
    ...
    #for hp training
    savepath=os.path.join("evaluation_errors2l", DEBUG_PATH, f"{SAVE_STRING}_mse_value_synth.csv")
    median=np.median(np.array(mse_errors_synth))
...
def main():
    hparams = {HP_LEARNING_RATE: learning_rate, HP_NOISE_LEVEL: noise_level,
    ...
    print(model.summary())
    testing_loss(model)
    model.save(os.path.join("models2l/", DEBUG_PATH, f"{SAVE_STRING}"))
