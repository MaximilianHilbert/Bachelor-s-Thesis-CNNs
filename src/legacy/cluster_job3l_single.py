...
keras_mlp=def_model.keras_model
...
num_units=(np.floor((np.floor((np.floor((N_REFL-kernel_size_1+1)/2)-kernel_size_2+1)/2)-kernel_size_3+1)/2))*num_filter_3*0.1
...
def testing_loss(model):
    ...
    # #for hp training
    # savepath=os.path.join("evaluation_errors3l", DEBUG_PATH, f"{SAVE_STRING}_mse_value_synth.csv")
...
def main():
    ...
    print(model.summary())
    testing_loss(model)
    model.save(os.path.join("models3l/", DEBUG_PATH, f"{SAVE_STRING}"))
